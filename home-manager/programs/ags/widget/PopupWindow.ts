...
export function Padding(name: string, {
  css = '',
  hexpand = true,
  vexpand = true,
}: EventBoxProps = {}) {
...
function PopupRevealer(name: string, child: Child, transition: Transition = 'slide_down') {
  return Widget.Box(
    { css: 'padding: 1px;' },
    Widget.Revealer({
      transition,
...
function Layout(name: string, child: Child, transition?: Transition) {
  return {
    'center': () => Widget.CenterBox({}, Padding(name), Widget.CenterBox(
      { vertical: true },
      Padding(name),
      PopupRevealer(name, child, transition),
      Padding(name),
    ), Padding(name)),
    'top': () => Widget.CenterBox({}, Padding(name), Widget.Box(
      { vertical: true },
      PopupRevealer(name, child, transition),
      Padding(name),
    ), Padding(name)),
